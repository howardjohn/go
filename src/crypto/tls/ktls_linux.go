...
func init() {
	...
	if err != nil {
		log.Printf("kTLS: kernel tls module not enabled")
		return
	}
	kTLSSupport = true
	...
	if err := syscall.Uname(&uname); err != nil {
		log.Printf("kTLS: call uname failed %v", err)
		return
	}
	...
	if err != nil {
		log.Printf("kTLS: parse major release failed %v", err)
		return
	}
	minor, err := strconv.Atoi(minorRelease)
	if err != nil {
		log.Printf("kTLS: parse minor release failed %v", err)
		return
	}
...
func (c *Conn) writeToFile(f *os.File, remain int64) (written int64, err error, handled bool) {
	...
	if offset+remain > fi.Size() {
		...
		if err != nil {
			log.Printf("file truncate error: %s", err)
			return 0, nil, false
		}
	}
...
func (c *Conn) enableKernelTLS(cipherSuiteID uint16, inKey, outKey, inIV, outIV []byte) error {
	...
	case TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256, TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256, TLS_RSA_WITH_AES_128_GCM_SHA256:
		log.Println("try to enable kernel tls AES_128_GCM")
	...
	case TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384, TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384, TLS_RSA_WITH_AES_256_GCM_SHA384:
		...
		log.Println("try to enable kernel tls AES_256_GCM")
...
func ktlsSendCtrlMessage(c *net.TCPConn, typ recordType, b []byte) (int, error) {
	...
	err0 := rwc.Write(func(fd uintptr) bool {
		...
		if err != nil {
			log.Println("kTLS: sendmsg failed:", err)
		}
		return true
	})
	if err0 != nil {
	...
	//log.Printf("kTLS: sendmsg, type: %d, payload len: %d", typ, len(b))
	return n, err
}
